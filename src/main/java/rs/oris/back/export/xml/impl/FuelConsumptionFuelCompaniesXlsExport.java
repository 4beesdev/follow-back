...
public class FuelConsumptionFuelCompaniesXlsExport extends XlsExporter {
    ...
    public FuelConsumptionFuelCompaniesXlsExport(LocalDateTime from, LocalDateTime to, Double maxSpeed, Double maxWheelSpin) {
        this.from=from;
        this.to=to;
    ...
    public <T> byte[] export(List<T> content, Class<T> myclass, String title) {
        ...
        try (Workbook workbook = new XSSFWorkbook()) {
            ...
            String subtitle="Od: "+from.format(formatter)+"     Do: "+to.format(formatter);
            ...
            headerRow.createCell(0).setCellValue("Granična brzina");
            headerRow.createCell(1).setCellValue("Granični broj obrtaja");

             headerRow = sheet.createRow(3);
            headerRow.createCell(0).setCellValue(roundDecimalNumber(maxSpeed,0));
            headerRow.createCell(1).setCellValue(roundDecimalNumber(maxWheelSpin,0));
            ...
            for (T obj : content) {
                ...
                for (Field field : obj.getClass().getDeclaredFields()) {
                    if (field.isAnnotationPresent(XlsTableElement.class)) {
                        ...
                        System.out.println("usao sam ovde 3");
                        if (field.isAnnotationPresent(TimestampFormat.class) && value instanceof Long) {
                            ...
                            System.out.println("usao sam ovde 2");
